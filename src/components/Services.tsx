...
export function Services({ onOpenBooking }: ServicesProps) {
  ...
  return (
    <section id="services" className="relative overflow-hidden bg-white dark:bg-gray-950 py-16 lg:py-28">
              ...
              >
                <div
                  className={`mb-6 flex h-12 w-12 items-center justify-center rounded-2xl transition-transform duration-300 group-hover:scale-110 ${
                    isYellow ? 'bg-gray-900' : isDark ? 'bg-gold-400' : 'bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-100 dark:ring-gray-700'
                  }`}
                >
                ...
                <h3 className={`font-display text-xl font-bold ${isYellow ? 'text-gray-900' : isDark ? 'text-white' : 'text-gray-900 dark:text-white'}`}>
                  {title}
                </h3>
                <p className={`mt-2 text-sm leading-relaxed ${isYellow ? 'text-gray-900/70' : isDark ? 'text-gray-400' : 'text-gray-500 dark:text-gray-400'}`}>
                  {t(descKey)}
                </p>
                ...
                <button
                  ...
                  className={`mt-6 inline-flex min-h-[44px] items-center gap-2 text-sm font-bold transition-colors ${
                    isYellow ? 'text-gray-900 hover:text-gray-700' : isDark ? 'text-gold-400 hover:text-gold-300' : 'text-gray-900 dark:text-white hover:text-gold-600 dark:hover:text-gold-400'
                  }`}
                >
                  {t('services.book')}
