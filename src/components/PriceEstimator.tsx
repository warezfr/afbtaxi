...
export function PriceEstimator({ onOpenBooking }: Props) {
  ...
  const toggleCls = (active: boolean) =>
    `flex min-h-[44px] flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm font-bold transition-colors ${
      active ? 'bg-gray-900 text-gold-400' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
    }`;

  return (
    <div data-testid="price-estimator" className="relative overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-[0_25px_60px_-30px_rgba(17,24,39,0.35)]">
      ...
      <div className="grid gap-5 p-6 sm:p-7 lg:grid-cols-[1fr_1fr_auto] lg:items-end lg:gap-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:col-span-2">
          <div>
            <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-gray-400">{t('estimator.from')}</label>
            <div className="flex min-h-[48px] items-center gap-2.5 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3">
              <Navigation size={16} className="shrink-0 text-gold-600" />
              <span className="text-sm font-bold text-gray-900">Fontainebleau</span>
            </div>
          </div>
          <div>
            <label htmlFor="estimator-dest" className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-gray-400">{t('estimator.to')}</label>
            <div className="relative">
              ...
              <select
                ...
                className="min-h-[48px] w-full appearance-none rounded-xl border border-gray-200 bg-white py-3 pl-11 pr-8 text-sm font-bold text-gray-900 transition-colors focus:border-gold-500 focus:outline-none focus:ring-2 focus:ring-gold-400/40"
              >
        ...
        {/* Price + CTA */}
        <div className="flex flex-col items-stretch gap-3 border-t border-gray-100 pt-5 lg:min-w-[220px] lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
          <div className="flex items-baseline justify-between lg:flex-col lg:items-start">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-400">{t('estimator.title')}</span>
            <span data-testid="estimator-price" className="font-display text-4xl font-black text-gray-900">
              {price ?? '— €'}
            </span>
          </div>
          <button
          ...
          <p className="text-[11px] leading-snug text-gray-400">{t('estimator.note')}</p>
        </div>
      </div>
    </div>
  );
