        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {VEHICLES.map((vehicle) => {
            ...
            return (
              <article
                key={vehicle.name}
                className="group relative overflow-hidden rounded-3xl border border-white/5 bg-white/[0.02] transition-all duration-500 hover:-translate-y-2 hover:border-gold-400/30 hover:shadow-[0_20px_60px_rgba(255,208,59,0.08)]"
              >
                <div className="relative h-52 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-5">
                    <div className="flex items-center gap-2">
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gold-400 text-ink">
                        <Icon size={18} />
                      </div>
                      <div>
                  ...
                  <ul className="space-y-2.5">
                    {vehicle.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-2.5 text-sm text-neutral-400">
                        <Check size={14} className="shrink-0 text-gold-400" />
                        {feature}
                      </li>
                    ))}
                  </ul>
